...
function checkStop() {
    ...
    let bush = document.querySelector(".tree")
    let bw = parseInt(window.getComputedStyle(tree).width)
    let bh = parseInt(window.getComputedStyle(tree).height)
    ...
    if (handX > parseInt(window.getComputedStyle(field).width)) {
        ...
        let bush = document.querySelector(".tree")
        let bw = parseInt(window.getComputedStyle(tree).left) + 100 + `px`;
        let bh = parseInt(window.getComputedStyle(tree).top)
    }
} 